# --- HPC BASELINE OPTIMIZATION SCRIPT ---
import os
import numpy as np
...
print("Generating Sim-to-Real Test Sets...")
...
# --- HYPERPARAMETERS FOR BASELINE RUN ---
EPOCHS = 40 
N_TRIALS = 50
SEEDS = [42, 123, 999, 8888]
# -----------------------------------------
...
def objective(trial):
    ...
    # STRIPPED: NO AUGMENTATION FACTOR FOR BASELINE!
    ...
    for i, seed in enumerate(SEEDS):
        ...
        def augment(image, label):
            # purely structural horizontal augmentation, NO Gausssian noise injected into standard pipeline!
        ...
        history = model.fit(
            trial_dataset,
            epochs=EPOCHS,
            validation_data=val_dataset_clean, # BASELINE EVALUATES STRICTLY AGAINST CORRUPTION-FREE DATA!
            verbose=2, 
            callbacks=callbacks
        )
...
print(f"Starting Baseline Bayesian Optimization with {N_TRIALS} trials over {EPOCHS} epochs each...")
study = optuna.create_study(direction="maximize", sampler=optuna.samplers.TPESampler(seed=42))
study.optimize(objective, n_trials=N_TRIALS, show_progress_bar=False)

print("Baseline Best Trial:")
print("  Average Accuracy:", study.best_trial.value)
print("  Best Hyperparameters:", study.best_trial.params)

optuna_results_path = os.path.join(OUTPUT_DIR, 'baseline_optuna_results.csv')
...
try:
    ...
    plt.tight_layout()
    optimization_history_path = os.path.join(OUTPUT_DIR, 'baseline_optimization_history.png')
    plt.savefig(optimization_history_path)
    
    fig2 = optuna.visualization.matplotlib.plot_param_importances(study)
    plt.tight_layout()
    hyperparameter_importances_path = os.path.join(OUTPUT_DIR, 'baseline_hyperparameter_importances.png')
...
for seed in SEEDS:
    print(f"\nTraining final optimal STANDARD benchmark model with seed {seed}... (Evaluating Robustness Variance)")
    keras.utils.set_random_seed(seed)
...
print(f"\nBASELINE O.P. EVALUATION STATS (Averaged across {len(SEEDS)} seeds):")
...
plt.errorbar(['Clean', 'Low Noise', 'Med Noise', 'High Noise'], means, yerr=stds, marker='o', linestyle='-', color='r', capsize=5)
plt.title('Standard Optimized Baseline Model: Accuracy vs. Sim-to-Real Sensor Noise')
plt.ylabel('Average Accuracy')
plt.grid(True)
final_accuracy_dropoff_path = os.path.join(OUTPUT_DIR, 'baseline_accuracy_dropoff.png')
plt.savefig(final_accuracy_dropoff_path)
print(f"Saved baseline accuracy drop-off chart to {final_accuracy_dropoff_path}")

print("HPC BASELINE COMPLETED SUCCESSFULLY.")
